            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    if(self.currently_selected[0] != "" and self.currently_selected[1] != ""):
                        p1_selected = Player(1, self.game, str(self.player1), (.4,.5), .1, .1, self.characters_display[self.player1])
                        p2_selected = Player(2, self.game, str(self.player2), (.6,.5), .075, .05, self.characters_display[self.player2])
                        self.level_select_state = LevelSelection(self.game)
